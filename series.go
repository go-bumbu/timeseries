...
// Series is a named time series with a single precision and retention.
type Series struct {
	Name      string
...
// DefineSeries creates or updates a series by name.
...
// GetSeries returns a series by name.
func (s *Store) GetSeries(name string) (Series, error) {
	...
	return Series{Name: row.Name, Precision: row.Precision, Retention: row.Retention}, nil
}
