...
func TestDefineSeries_FieldSync(t *testing.T) {
	for _, tdb := range testdbs.DBs() {
		t.Run(tdb.DbType(), func(t *testing.T) {
			...
			if err := s.Write(context.Background(), "S", Point{Time: day, Values: map[string]float64{"a": 1, "b": 2}}); err != nil {
				t.Fatal(err)
			}
			...
			got, err := s.GetSeries(context.Background(), "S")
			if err != nil {
				t.Fatal(err)
			}
			if len(got.Fields) != 2 {
			...
			if _, found, _ := s.FieldAt(context.Background(), "S", "a", day); !found {
				t.Fatal("field a record should survive the sync")
			}
			if err := s.Write(context.Background(), "S", Point{Time: day, Values: map[string]float64{"b": 9}}); err == nil {
...
func TestDefineSeries_UnknownAggregate(t *testing.T) {
	for _, tdb := range testdbs.DBs() {
		t.Run(tdb.DbType(), func(t *testing.T) {
			...
			err = s.DefineSeries(context.Background(), Series{
